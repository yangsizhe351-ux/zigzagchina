...
- 侧边栏最多保留 6 个长期角色窗口：唯一总控、文本与翻译、政策与合规、网站结构与实现、图像生成、集成验收。安全总控换代时前任与继任者可短暂重叠；继任者双重核验后立即归档前任。诊断和一次性复审完成后立即可恢复归档，不为每个小问题新建窗口。
- Fast、模型与思考深度只由总控按 `TASKS.yaml#role_registry` 调度；专项窗口不得自行切换或静默降档。
- 当总控出现上下文压缩、反复重建状态或持续卡顿时，按 `TASKS.yaml#control_handoff_policy` 和全局 `$control-context-handoff` 执行交接。只有新总控核验项目、`main`、HEAD、工作区和台账后，旧总控才可做可恢复归档。
...
- 每个任务轮次只在入场时完整读取一次 `AGENTS.md` 与 `HANDOFF.md`；随后只读 `TASKS.yaml` 顶部策略、当前 issue 和发生变化的目标段。只有 HEAD、相关治理文件或上下文压缩发生变化时才重读完整入口。
- 同一候选 SHA 与同一组验收标准只允许一个 `qa-gate` owner；没有新 SHA、验收标准变化或不完整结论时，不得重复启动新验收或创建复审链。
- 跟进任务最多做一次即时状态快照，之后使用事件驱动的长等待；禁止 30–60 秒固定轮询和重复读取完整任务历史。
- 总控交接只传递 issue ID、基线 SHA、候选 SHA、变更路径、失败 AC 或结果、下一动作；不重复粘贴完整 `WORKFLOW.md`、`TASKS.yaml`、`HANDOFF.md` 或长日志。
- owner 先运行最小机械检查，只有冻结候选才进入独立 QA；失败后先定位根因并只重跑失败项，完整验证矩阵在最终候选上执行一次。
- 模型与风险边界仍以 `TASKS.yaml` 为准；效率优化不得降低政策、安全、共享架构、独立验收或发布闸门的既定档位。
